...
매일 한국시간 09:00 / 12:00 / 18:00 / 21:00, 지정한 키워드(AI, 로보틱스, 정책자금, IPO, IP/K-Culture 등) 관련
...
src/dart_collector.py     # DART 전자공시 API (IPO/투자/증자 등 관련 공시)
...
이후로는 매일 09/12/18/21시(KST)에 자동으로 실행됩니다.
...
- **정책보고서/증권사 리서치(연구소, 증권사 등)**: 대부분 RSS를 공식 제공하지 않아 이번 버전에는
  뉴스 검색 결과 위주로만 반영되어 있습니다. 특정 기관(예: KOCCA, 삼성증권 리서치)의 발간 페이지가
  RSS나 API를 제공하면 `news_collector.py`에 소스를 추가해 확장할 수 있습니다 — 필요하시면 이어서 작업해드릴게요.
- **API 무료 쿼터**: 네이버 뉴스 API, DART API 모두 일일 호출 한도가 있어 키워드 수가 많으면
  하루 4회 실행 시 한도에 근접할 수 있습니다. 실제 운영 중 초과 시 카테고리별 실행 주기를 조정하는 게 좋습니다.
